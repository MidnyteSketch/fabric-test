...
public final class PatchesCuriosityGoal extends Goal {
    ...
    private boolean tryUpgradeTarget() {
        ...
        if (targetPriority == PatchesCuriosityPriority.LOW) {
            Axolotl axolotl = findNearbyAxolotl();
            if (axolotl != null) { report("PRIORITY", "An Axolotl is more interesting than the flower; switching targets."); selectAxolotl(axolotl); beginNotice(); return true; }
        }
        return false;
    }
    ...
    private void finish(boolean remember) {
        ...
        patches.getNavigation().stop(); patches.clearActivityExpression(); targetKind = null; targetPriority = null; blockTarget = null; axolotlTarget = null;
