...
import com.midnyte.patches.entity.ai.PatchesFlowerCuriosityGoal;
...
public final class PatchesEntity extends PathfinderMob {
    ...
    private static final int COOKIE_LAUGH_TICKS=20,OTHER_FOOD_TICKS=12,HURT_FACE_TICKS=12;
    ...
    public static AttributeSupplier.Builder createAttributes(){return PathfinderMob.createMobAttributes().add(Attributes.MAX_HEALTH,10.0).add(Attributes.MOVEMENT_SPEED,0.28).add(Attributes.TEMPT_RANGE,10.0).add(Attributes.FOLLOW_RANGE,20.0);}

    @Override protected void registerGoals(){this.goalSelector.addGoal(0,new FloatGoal(this));this.goalSelector.addGoal(1,new PatchesFlowerCuriosityGoal(this));this.goalSelector.addGoal(2,new PatchesFollowGoal(this));this.goalSelector.addGoal(3,new PatchesTemptGoal(this,1.0,Ingredient.of(Items.COOKIE),false));this.goalSelector.addGoal(5,new RandomStrollGoal(this,0.85));this.goalSelector.addGoal(6,new LookAtPlayerGoal(this,Player.class,8.0F));this.goalSelector.addGoal(7,new RandomLookAroundGoal(this));}
    @Override protected void defineSynchedData(SynchedEntityData.Builder builder){super.defineSynchedData(builder);builder.define(MODE,PatchesMode.WANDERING.id());builder.define(BUNDLE,ItemStack.EMPTY);builder.define(SPYGLASS,ItemStack.EMPTY);builder.define(EXPRESSION,PatchesExpression.DEFAULT.id());}
    ...
    public ItemStack getBundleStack(){return this.entityData.get(BUNDLE);} public boolean hasBundle(){return !getBundleStack().isEmpty();}
    ...
    public ItemStack getSpyglassStack(){return this.entityData.get(SPYGLASS);} public boolean hasSpyglass(){return !getSpyglassStack().isEmpty();}
    ...
    @Override protected InteractionResult mobInteract(Player player,InteractionHand hand){ItemStack stack=player.getItemInHand(hand);if(BundleSupport.isBundle(stack)){if(hasBundle())return InteractionResult.FAIL;if(!level().isClientSide()){setBundleStack(stack.copyWithCount(1));if(!player.hasInfiniteMaterials())stack.shrink(1);}return InteractionResult.SUCCESS;}if(stack.is(Items.SPYGLASS)){if(hasSpyglass())return InteractionResult.FAIL;if(!level().isClientSide()){setSpyglassStack(stack.copyWithCount(1));if(!player.hasInfiniteMaterials())stack.shrink(1);}return InteractionResult.SUCCESS;}if(stack.isEmpty()&&player.isShiftKeyDown()){if(hasSpyglass()){if(!level().isClientSide()){ItemStack equipped=getSpyglassStack().copy();setSpyglassStack(ItemStack.EMPTY);returnOrDrop(player,equipped);}return InteractionResult.SUCCESS;}if(hasBundle()){if(!level().isClientSide()){ItemStack equipped=getBundleStack().copy();setBundleStack(ItemStack.EMPTY);returnOrDrop(player,equipped);}return InteractionResult.SUCCESS;}}if(stack.is(Items.COOKIE)){if(!level().isClientSide()){setFollowingPlayer(player);if(getMode()==PatchesMode.WANDERING)setMode(PatchesMode.FOLLOWING);else if(getMode()==PatchesMode.FOLLOWING)setMode(PatchesMode.WANDERING);heal(2.0F);consumeOne(player,stack);setTimedExpression(PatchesExpression.LAUGH,COOKIE_LAUGH_TICKS);playSound(SoundEvents.GENERIC_EAT.value(),0.7F,1.15F);}return InteractionResult.SUCCESS;}if(stack.isEmpty()){if(!level().isClientSide()){if(getMode()==PatchesMode.SITTING)setMode(modeBeforeSitting);else{modeBeforeSitting=getMode();setMode(PatchesMode.SITTING);}}return InteractionResult.SUCCESS;}if(isLikedFood(stack)&&getHealth()<getMaxHealth()){if(!level().isClientSide()){float healing=stack.is(Items.MUSHROOM_STEW)?6.0F:stack.is(Items.APPLE)?3.0F:2.0F;heal(healing);boolean stew=stack.is(Items.MUSHROOM_STEW);consumeOne(player,stack);setTimedExpression(PatchesExpression.MOUTH_OPEN,OTHER_FOOD_TICKS);if(stew&&!player.hasInfiniteMaterials())returnOrDrop(player,new ItemStack(Items.BOWL));playSound(SoundEvents.GENERIC_EAT.value(),0.7F,1.05F);}return InteractionResult.SUCCESS;}if(isRejectedFood(stack))return InteractionResult.FAIL;return super.mobInteract(player,hand);}
    ...
    private String encodeFlowerMemory(){StringBuilder encoded=new StringBuilder();for(BlockPos pos:rememberedFlowerCuriosities){if(!encoded.isEmpty())encoded.append(';');encoded.append(pos.getX()).append(',').append(pos.getY()).append(',').append(pos.getZ());}return encoded.toString();}
    private void decodeFlowerMemory(String encoded){rememberedFlowerCuriosities.clear();if(encoded.isEmpty())return;for(String entry:encoded.split(";")){String[] xyz=entry.split(",");if(xyz.length!=3)continue;try{rememberedFlowerCuriosities.add(new BlockPos(Integer.parseInt(xyz[0]),Integer.parseInt(xyz[1]),Integer.parseInt(xyz[2])));}catch(NumberFormatException ignored){}}}
    @Override protected void addAdditionalSaveData(ValueOutput output){super.addAdditionalSaveData(output);output.putInt("PatchesMode",getMode().id());output.putInt("PatchesModeBeforeSitting",modeBeforeSitting.id());output.storeNullable("PatchesFollowingPlayer",UUIDUtil.CODEC,followingPlayerUuid);ItemStack bundle=getBundleStack();if(!bundle.isEmpty())output.store("PatchesBundle",ItemStack.CODEC,bundle);ItemStack spyglass=getSpyglassStack();if(!spyglass.isEmpty())output.store("PatchesSpyglass",ItemStack.CODEC,spyglass);output.putString("PatchesRememberedFlowers",encodeFlowerMemory());}
    @Override protected void readAdditionalSaveData(ValueInput input){super.readAdditionalSaveData(input);setMode(PatchesMode.fromId(input.getIntOr("PatchesMode",PatchesMode.WANDERING.id())));modeBeforeSitting=PatchesMode.fromId(input.getIntOr("PatchesModeBeforeSitting",PatchesMode.WANDERING.id()));followingPlayerUuid=input.read("PatchesFollowingPlayer",UUIDUtil.CODEC).orElse(null);ItemStack savedBundle=input.read("PatchesBundle",ItemStack.CODEC).orElse(ItemStack.EMPTY);setBundleStack(savedBundle.isEmpty()||BundleSupport.isBundle(savedBundle)?savedBundle:ItemStack.EMPTY);ItemStack savedSpyglass=input.read("PatchesSpyglass",ItemStack.CODEC).orElse(ItemStack.EMPTY);setSpyglassStack(savedSpyglass.isEmpty()||savedSpyglass.is(Items.SPYGLASS)?savedSpyglass:ItemStack.EMPTY);decodeFlowerMemory(input.getStringOr("PatchesRememberedFlowers",""));expressionOverrideTicks=0;updateExpression();}
}
