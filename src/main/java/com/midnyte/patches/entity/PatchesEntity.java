...
public final class PatchesEntity extends PathfinderMob {
    ...
    private void setBundleStack(ItemStack stack) {
        if (!stack.isEmpty() && !BundleSupport.isBundle(stack)) throw new IllegalArgumentException("Patches can only equip a Bundle");
    ...
    private void setSpyglassStack(ItemStack stack) {
        if (!stack.isEmpty() && !stack.is(Items.SPYGLASS)) throw new IllegalArgumentException("Patches can only equip a Spyglass in his discovery-tool slot");
    ...
    public @Nullable Player getFollowingPlayer() {
        if (followingPlayerUuid == null) return null;
    ...
    private void updateExpression() {
        if (expressionOverrideTicks > 0) { expressionOverrideTicks--; return; }
        if (getMode() == PatchesMode.SITTING) { setExpression(PatchesExpression.RESTING); return; }
        Player temptingPlayer = level().getNearestPlayer(this, COOKIE_NOTICE_RANGE);
        if (temptingPlayer != null && playerIsHoldingCookie(temptingPlayer)) { setExpression(PatchesExpression.SURPRISED); return; }
        setExpression(PatchesExpression.DEFAULT);
    }

    private static boolean playerIsHoldingCookie(Player player) {
        return player.getMainHandItem().is(Items.COOKIE) || player.getOffhandItem().is(Items.COOKIE);
    }

    @Override
    public boolean hurtServer(ServerLevel level, DamageSource source, float amount) {
        boolean damaged = super.hurtServer(level, source, amount);
        if (damaged) setTimedExpression(PatchesExpression.HURT, HURT_FACE_TICKS);
        return damaged;
    }
    ...
    protected void dropCustomDeathLoot(ServerLevel level, DamageSource source, boolean killedByPlayer) {
        ...
        if (!bundle.isEmpty()) { setBundleStack(ItemStack.EMPTY); spawnEquippedDrop(level, bundle); }
        ItemStack spyglass = getSpyglassStack().copy();
        if (!spyglass.isEmpty()) { setSpyglassStack(ItemStack.EMPTY); spawnEquippedDrop(level, spyglass); }
    }
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (BundleSupport.isBundle(stack)) {
            if (hasBundle()) return InteractionResult.FAIL;
            if (!level().isClientSide()) { setBundleStack(stack.copyWithCount(1)); if (!player.hasInfiniteMaterials()) stack.shrink(1); }
            return InteractionResult.SUCCESS;
        }
        if (stack.is(Items.SPYGLASS)) {
            if (hasSpyglass()) return InteractionResult.FAIL;
            if (!level().isClientSide()) { setSpyglassStack(stack.copyWithCount(1)); if (!player.hasInfiniteMaterials()) stack.shrink(1); }
            return InteractionResult.SUCCESS;
        }
        if (stack.isEmpty() && player.isShiftKeyDown()) {
            if (hasSpyglass()) {
                if (!level().isClientSide()) { ItemStack equipped = getSpyglassStack().copy(); setSpyglassStack(ItemStack.EMPTY); returnOrDrop(player, equipped); }
                return InteractionResult.SUCCESS;
            }
            if (hasBundle()) {
                if (!level().isClientSide()) { ItemStack equipped = getBundleStack().copy(); setBundleStack(ItemStack.EMPTY); returnOrDrop(player, equipped); }
                return InteractionResult.SUCCESS;
            }
        }
        if (stack.is(Items.COOKIE)) {
            if (!level().isClientSide()) {
                ...
                if (getMode() == PatchesMode.WANDERING) setMode(PatchesMode.FOLLOWING);
                else if (getMode() == PatchesMode.FOLLOWING) setMode(PatchesMode.WANDERING);
                heal(2.0F); consumeOne(player, stack); setTimedExpression(PatchesExpression.LAUGH, COOKIE_LAUGH_TICKS);
        ...
        if (stack.isEmpty()) {
            if (!level().isClientSide()) {
                if (getMode() == PatchesMode.SITTING) setMode(modeBeforeSitting);
                else { modeBeforeSitting = getMode(); setMode(PatchesMode.SITTING); }
            }
            return InteractionResult.SUCCESS;
        }
        if (isLikedFood(stack) && getHealth() < getMaxHealth()) {
            if (!level().isClientSide()) {
                ...
                if (stew && !player.hasInfiniteMaterials()) returnOrDrop(player, new ItemStack(Items.BOWL));
    ...
    private void returnOrDrop(Player player, ItemStack stack) {
        ...
        ItemEntity dropped = new ItemEntity(level(), player.getX(), player.getY() + 0.5, player.getZ(), stack);
        dropped.setDefaultPickUpDelay();
        level().addFreshEntity(dropped);
    }
    ...
    private String encodeFlowerMemory() {
        StringBuilder encoded = new StringBuilder();
        for (BlockPos pos : rememberedFlowerCuriosities) {
    ...
    private void decodeFlowerMemory(String encoded) {
        rememberedFlowerCuriosities.clear();
        if (encoded.isEmpty()) return;
        for (String entry : encoded.split(";")) {
            String[] xyz = entry.split(",");
            if (xyz.length != 3) continue;
            try { rememberedFlowerCuriosities.add(new BlockPos(Integer.parseInt(xyz[0]), Integer.parseInt(xyz[1]), Integer.parseInt(xyz[2]))); }
            catch (NumberFormatException ignored) { }
        }
    }

    private String encodeAxolotlMemory() {
        StringBuilder encoded = new StringBuilder();
        for (UUID uuid : rememberedAxolotlCuriosities) {
            if (!encoded.isEmpty()) encoded.append(';');
            encoded.append(uuid);
        }
        return encoded.toString();
    ...
    private void decodeAxolotlMemory(String encoded) {
        rememberedAxolotlCuriosities.clear();
        if (encoded.isEmpty()) return;
        for (String entry : encoded.split(";")) {
            try { rememberedAxolotlCuriosities.add(UUID.fromString(entry)); }
    ...
    protected void addAdditionalSaveData(ValueOutput output) {
        ...
        if (!bundle.isEmpty()) output.store("PatchesBundle", ItemStack.CODEC, bundle);
        ItemStack spyglass = getSpyglassStack();
        if (!spyglass.isEmpty()) output.store("PatchesSpyglass", ItemStack.CODEC, spyglass);
        output.putString("PatchesRememberedFlowers", encodeFlowerMemory());
    ...
    protected void readAdditionalSaveData(ValueInput input) {
        ...
        setBundleStack(savedBundle.isEmpty() || BundleSupport.isBundle(savedBundle) ? savedBundle : ItemStack.EMPTY);
        ItemStack savedSpyglass = input.read("PatchesSpyglass", ItemStack.CODEC).orElse(ItemStack.EMPTY);
        setSpyglassStack(savedSpyglass.isEmpty() || savedSpyglass.is(Items.SPYGLASS) ? savedSpyglass : ItemStack.EMPTY);
        decodeFlowerMemory(input.getStringOr("PatchesRememberedFlowers", ""));
