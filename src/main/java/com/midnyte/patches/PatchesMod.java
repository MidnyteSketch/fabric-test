        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) ->
                dispatcher.register(Commands.literal("patches_debug_trader")
                        .executes(context -> {
                            ...
                            WanderingTrader trader = EntityTypes.WANDERING_TRADER.spawn(level, traderPos, EntitySpawnReason.COMMAND);
                            if (trader == null) return 0;
                            trader.setDespawnDelay(48000);

                            for (int i = 0; i < 2; i++) {
                                var llamaPos = traderPos.offset(i == 0 ? -2 : 2, 0, 2);
                                TraderLlama llama = EntityTypes.TRADER_LLAMA.spawn(level, llamaPos, EntitySpawnReason.COMMAND);
